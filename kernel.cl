...
unsigned long long keccak256_maxcoin_opt_v(global unsigned long long *data)
{
	int j;
	sph_u64 t;
	DECL64(c0);
...
kernel void maxcoin_process(global uint* in, global uint* out, uint begin_nonce, uint target) {
	size_t id = get_global_id(0);
	uint nonce = (uint)id + begin_nonce;

	in[19]=nonce;

	if(keccak256_maxcoin_opt_v((unsigned long long*)in) <= target)
	{
		uint pos = atomic_inc(out) + 1;
		out[pos] = nonce;
	}
